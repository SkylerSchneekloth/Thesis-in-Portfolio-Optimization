...
clusterData_MVO_2pt5 <- read.csv("clusterData_MVO_2pt5.csv", header = TRUE)
...
if (returnsType == 1) {
  ...
  returns <- returns[, colnames(returns) %in% clusterData_MVO_2pt5$Ticker]
...
node_data <- data.frame(
  ...
  weight = clusterData_MVO_2pt5$Weight[match(rownames(assetCorrelations), clusterData_MVO_2pt5$Ticker)]
)
