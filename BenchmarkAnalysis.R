...
#Risk-Adjusted Returns
  # Calculate cap-weighted benchmarks
    fees <- 0.0025
    df$spTransactionCosts <- df$spTurnover * fees
    df$vwretd <- df$vwretd - df$spTransactionCosts
  # SHARPE RATIO (CFA GIPS standard for "ex post" SR)
    # Calculate the SR numerator
      meanMarketReturn <- rollapply(df$vwretd, width=12, FUN = mean, fill=NA, align="right")
      meanRiskFreeReturn <- rollapply(df$t30ret, width=12, FUN = mean, fill=NA, align="right")
    ...
    # Calculate the SR denominator as the standard deviation of portfolio returns over prior 12 months
      SRdemoninator <- rollapply(df$vwretd, width=12, FUN = sd, fill=NA, align="right")
    # Calculate the monthly Sharpe Ratio
      df["spSR_cw"] <- SRnumerator / SRdemoninator
  # TREYNOR RATIO
      df["spTR_cw"] <- SRnumerator / 1 #the S&P's market beta is equal to 1
