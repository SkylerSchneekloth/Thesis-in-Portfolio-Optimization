...
p <- ggplot(all_data, aes(x = ES, y = mean, group = DataFrame, color = DataFrame)) +
  ...
  theme(
    ...
    text = element_text(size = 18),       # Set base text size to 18
...
p <- ggplot(long_allocations_df, aes(x = factor(Row), y = Value, fill = Ticker)) +
  ...
  theme(
    legend.position = "right",
    axis.text.x = element_text(angle = 45, hjust = 1, size = 18), # Rotate x labels and increase size
...
setwd("C:/Users/Skyler/OneDrive - University of Iowa/FIN9391 Thesis in Equity Portfolio Optimization/RESULTS_no transaction costs")
...
G <- G[nrow(G),]  # Select the most recent observation
H <- H[nrow(H),]  # Select the most recent observation
