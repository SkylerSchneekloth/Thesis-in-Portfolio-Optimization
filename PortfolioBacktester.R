...
#VARY THESE PARAMETERS TO PERFORM BACKTESTS:
  ...
  optimizer <- 3                      #1 Random weight w/ (LONG ONLY) w/ base R
  ...
  ##############################################################
  #Min and Max constraints do not apply to optimizers #1, #2, #4
  #gamma only applies to optimizer #4
  #n_bootstraps only applies to optimizer #6
  ##############################################################
  
  MaxConstraint <- .025               #The maximum asset allocation per stock for MVO (some values may cause errors, if this happens simply relax the constraint and try again)
  ...
  gamma <- 75                         #The "coefficient of risk aversion" for shrinkage portfolio
  n_bootstraps <- 100                 #The number of bootstraps to resampled portfolio
...
df_weights <- matrix(data=NA, nrow=nrow(benchmarks_truncated), ncol=nrow(FullHistoricalTickerList))
df_weights <- data.frame(df_weights, row.names=rownames(benchmarks_truncated))
colnames(df_weights) <- FullHistoricalTickerList$V1
...
for (i in 1:(nrow(returns)-roller)){
  ...
  if (optimizer==1) {
    df_weights[i, names(random_portfolio)] <- random_portfolio #similar to HLOOKUP in Microsoft Excel
    portRet <- sum(as.vector(random_portfolio)*as.vector(returns_tmp2))
  } else if (optimizer==2 | optimizer==3) {
    df_weights[i, names(opt_portfolio$weights)] <- opt_portfolio$weights #similar to HLOOKUP in Microsoft Excel
    ...
    df_weights[i, rownames(AssetWeights)] <- AssetWeights[,1] #similar to HLOOKUP in Microsoft Excel
    portRet <- sum(as.vector(opt_portfolio$weights)*as.vector(returns_tmp2))
  } else if (optimizer==5) {
...
write.csv(df_weights, paste0(filename, "_weights.csv"))
